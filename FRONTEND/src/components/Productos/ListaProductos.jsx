...
export function ListaProductos() {
  ...
  const cargarProductos = async () => {
    try {
      ...
      if (response.data) setProductos(response.data);
      else throw new Error('No se recibieron datos de productos');
    } catch (error) {
      console.error(error);
  ...
  const handleEditar = (producto) => {
    ...
    navigate(`/producto/editar/${producto.id}`, { state: { producto: productoCompleto } });
  };
  ...
  const handleEliminar = async (idProducto) => {
    ...
    try {
      ...
      if (error.response?.status === 500) await cargarProductos();
    } finally {
      setLoading(false);
    }
  };
