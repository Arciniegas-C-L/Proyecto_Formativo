    ...
    <>
        ...
        <div style={{
          ...
          <div style={{
            ...
            <button onClick={handleGoToLogin} style={{
              background: '#222', color: '#fff', border: 'none', borderRadius: 8, padding: '0.6rem 1.5rem', marginRight: 10, cursor: 'pointer', fontWeight: 600
            }}>Ir a iniciar sesión</button>
            <button onClick={handleCloseModal} style={{
              background: '#eee', color: '#222', border: 'none', borderRadius: 8, padding: '0.6rem 1.5rem', cursor: 'pointer', fontWeight: 600
            }}>Cancelar</button>
          </div>
        </div>
      )}
        ...
        <div className="comentarios">
          ...
          {comentariosAleatorios.map((c) => {
            ...
            if (!c.usuario_avatar_seed) {
              console.warn('Comentario sin avatar_seed, id comentario:', c.id, 'usuario:', c.usuario_nombre, c.usuario_apellido);
            }
            return (
              <div className="usuario" key={c.id}>
                <div className="usuario-info">
                  {c.usuario_avatar_seed ? (
                    <img
                      src={generateAvatarUrlFromSnapshot(c.usuario_avatar_seed, c.usuario_avatar_options)}
                      alt={c.usuario_nombre}
                    />
                  ) : (
                    <div className="avatar-placeholder" title="Sin avatar disponible" />
                  )}
                  <div className="usuario-detalles">
                    <h3>{c.usuario_nombre} {c.usuario_apellido}</h3>
                ...
                <div className="comentario-usuario">
                  <p>"{c.texto}"</p>
                  <span className="fecha">{new Date(c.fecha).toLocaleDateString()}</span>
                </div>
              </div>
            );
          })}
      ...
      <section className="formulario-comentarios formulario-comentarios-flex">
        <div className="formulario-comentarios-formbox">
          ...
          <form className="form-comentario-home" autoComplete="off" ref={formRef} onSubmit={handleSubmit}>
            <div className="calificacion">
              <h3>Tu valoración</h3>
              <div className="estrellas-calificacion">
                <Radio valor={valoracion} onChange={handleValoracion} editable={true} />
              </div>
            </div>
            ...
            <button type="submit" disabled={loading || !comentario.trim()}>{loading ? 'Enviando...' : 'Enviar comentario'}</button>
          </form>
        </div>
