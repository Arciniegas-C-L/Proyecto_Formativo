...
class Notificacion(models.Model):
    ...
    metadata     = models.JSONField(default=dict, blank=True)  # p.ej. {inventario_id, producto_id, talla_id, cantidad, umbral}
    creado_en    = models.DateTimeField(auto_now_add=True)
    leido_en     = models.DateTimeField(null=True, blank=True)
    resuelto     = models.BooleanField(default=False)
    destinatario = models.ForeignKey('Usuario', null=True, blank=True, on_delete=models.SET_NULL)

    class Meta:
        ordering = ['-creado_en']
        indexes  = [models.Index(fields=['tipo', 'resuelto'])]
    ...
    def __str__(self):
        return f"[{self.tipo}] {self.titulo} ({'resuelta' if self.resuelto else 'activa'})"


class StockAlertActivo(models.Model):
    """
    Evita duplicados: mantiene 1 alerta activa por inventario cuando está bajo/agotado.
    Se borra automáticamente cuando el stock vuelve a estar OK.
    """
    inventario     = models.OneToOneField('Inventario', on_delete=models.CASCADE, related_name='alerta_activa', primary_key=True)
    tipo           = models.CharField(max_length=32, choices=Notificacion.TIPO_CHOICES)  # stock_low | stock_out
    umbral         = models.PositiveIntegerField(default=5)
    cantidad       = models.IntegerField(default=0)   # último valor observado
    creado_en      = models.DateTimeField(auto_now_add=True)
    actualizado_en = models.DateTimeField(auto_now=True)

    def __str__(self):
        return f"Alerta {self.tipo} inv#{self.inventario_id} ({self.cantidad})"
    
@receiver(post_save, sender=Inventario)
def alerta_stock_bajo(sender, instance: Inventario, **kwargs):
    """
    Genera/actualiza notificaciones cuando el stock (por talla) cae a <= umbral (5)
    o llega a 0. Si sube > umbral, limpia la alerta activa y marca notificaciones
    previas como resueltas.
    Usa stock_talla como fuente principal; si no, usa cantidad.
    """
    try:
        UMBRAL = 5
        # fuente principal de stock en tu app:
        stock = instance.stock_talla if instance.stock_talla is not None else instance.cantidad
        stock = int(stock)
        ...
        if stock <= 0:
            StockAlertActivo.objects.update_or_create(
                inventario=instance,
                defaults={'tipo': 'stock_out', 'umbral': UMBRAL, 'cantidad': stock}
            )
            Notificacion.objects.create(
                titulo=f"Sin stock: {instance.producto.nombre} - Talla {instance.talla.nombre}",
                mensaje=f"El inventario #{instance.idInventario} quedó en {stock}.",
                tipo='stock_out',
                metadata={
                    'inventario_id': instance.idInventario,
                    'producto_id': instance.producto_id,
                    'producto': instance.producto.nombre,
                    'talla_id': instance.talla_id,
                    'talla': instance.talla.nombre,
                    'cantidad': stock,
                    'umbral': UMBRAL,
                }
            )
        ...
        # 2) stock bajo (1..UMBRAL)
        if 0 < stock <= UMBRAL:
            StockAlertActivo.objects.update_or_create(
                inventario=instance,
                defaults={'tipo': 'stock_low', 'umbral': UMBRAL, 'cantidad': stock}
            )
            Notificacion.objects.create(
                titulo=f"Stock bajo: {instance.producto.nombre} - Talla {instance.talla.nombre}",
                mensaje=f"Quedan {stock} unidades (umbral {UMBRAL}).",
                tipo='stock_low',
                metadata={
                    'inventario_id': instance.idInventario,
                    'producto_id': instance.producto_id,
        ...
        # 3) stock OK (> UMBRAL): limpiar alerta activa y marcar notificaciones previas como resueltas
        alerta = getattr(instance, 'alerta_activa', None)
        if alerta:
            alerta.delete()
            Notificacion.objects.filter(
                metadata__inventario_id=instance.idInventario,
                tipo__in=['stock_low', 'stock_out'],
                resuelto=False
